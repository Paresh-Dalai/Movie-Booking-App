...
const updateUseerDetails = async (req,res) => {
     ...
     try {
        ...
        if (enteredUserId) {
            ...
            if (foundUser) {
                foundUser.name = req.body.name != undefined ? req.body.name : foundUser.name;
                foundUser.userType = req.body.name != undefined ? req.body.userType : foundUser.userType;
                foundUser.userStatus = req.body.userStatus != undefined ? req.body.userStatus : foundUser.userStatus;
                foundUser.email = req.body.email != undefined ? req.body.email : foundUser.email;
              
                let updatedUserData = await foundUser.save();
                res.send("userDetails updated Successfully..." + updatedUserData).status(200)
             ...
             res.send("Please provide a valid userId for update user Details...").status(400)
        }

     } catch(error) {
         res.send("error occured while updating user Details..." + error.message).status(500)
     }
}
...
const deleteUser = async (req,res) => {
    ...
    try {
        if (enteredUserId) {
            ...
            res.send("Please enter a userId for successfully remove userData...").status(400)
        }
